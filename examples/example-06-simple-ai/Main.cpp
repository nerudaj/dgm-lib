/**
 *  This example shows how you can build an AI with the help of dgm-fsm-lib and
 * native dgm-lib primitives like vision cone for visibility checks.
 *
 *  Blue circle is an AI on a patrol route and yellow circle is you. You can
 * move by using WASD. If you step into vision cone of an AI, it will start
 * hunting you, when it loses you, it'll return to its patrol route.
 *
 *  The vision detection isn't perfect, so it is not hard to lose the AI, but
 * that is besides the point of this example - the point is how to you can put
 * together a FSM using dgm-fsm-lib.
 */
...
int main(int argc, char* argv[])
{
    ...
    Scene scene = { .level = level, .player = player, .npc = npc };
