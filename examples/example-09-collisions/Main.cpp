...
dgm::Controller createSimpleController()
{
    dgm::Controller result;

    result.bindInput(Action::Up, sf::Keyboard::Key::W);
    result.bindInput(Action::Down, sf::Keyboard::Key::S);
    result.bindInput(Action::Left, sf::Keyboard::Key::A);
    result.bindInput(Action::Right, sf::Keyboard::Key::D);
...
int main()
{
    ...
    auto&& input = createSimpleController();
    dgm::Time time;
    ...
    while (window.isOpen())
    {
        ...
        forward = dgm::Math::toUnit(sf::Vector2f {
                      -input.getInputValue(Action::Left)
                          + input.getInputValue(Action::Right),
                      -input.getInputValue(Action::Up)
                          + input.getInputValue(Action::Down),
                  })
                  * time.getDeltaTime() * SPEED;
