...
public:
    virtual void update(const dgm::Time& time) override
    {
        ...
        for (auto& p : particles)
        {
            ...
            {
                const auto DELTA_FORWARD =
                    p->getForward() * time.getDeltaTime();
    ...
    void reset()
    {
        ...
        for (auto& p : particles)
        {
            const float randomSizeFactor =
                getRandomFloat(MIN_PARTICLE_SIZE, MAX_PARTICLE_SIZE);
            const float randomForwardFactor =
                getRandomFloat(MIN_EMITTER_FORCE, MAX_EMITTER_FORCE);
            const float randomSpawnOffset = getRandomFloat(0.f, EMITTER_RADIUS);
            const auto randomUnitVector = dgm::Math::toUnit(sf::Vector2f(
                getRandomFloat(-1.f, 1.f), getRandomFloat(-1.f, 1.f)));

            p->setColor(sf::Color::Red);
            p->setForward(randomUnitVector * randomForwardFactor);
            p->setForward(
                { p->getForward().x / 1.5f, p->getForward().y * 2.f });

            // We don't care about lifespan of individual particles in this
            // effect, so it can be zero
            p->spawn(
                emitterPosition + randomUnitVector * randomSpawnOffset,
                sf::Vector2f(1.f, 1.f) * randomSizeFactor,
                sf::Time::Zero);
        }
    }
    ...
    EffectBloodSpatter(
        ...
        const float floorYCoord)
        : ParticleEffectBase(particleCount)
        , emitterPosition(position)
        , floorY(floorYCoord)
    {
        // This effect initializes all particles at once
        // We need to loop over all allocated slots and "add" them to visible
        // part of the buffer
