...
namespace dgm
{
    ...
    private:
        void
        loadResourceFromFile(const std::string& filename, sf::Texture& texture);
    ...
    public:
        ...
        T& get(const std::string& id) const
        {
            if (not isResourceInDatabase(id))
            {
                throw dgm::GeneralException(
                    "Resource '" + id + "' is not in database.");
            }
         ...
         *  \param[in]  allowedExtensions  Extensions of files that will be
         * opened for loading. Other files will be ignored \param[in]  recursive
         * Whether load subdirectories (Default: false)
         */
        template<SupportedResourceType T>
