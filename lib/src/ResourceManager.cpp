...
void ResourceManager::loadResourceFromFile(
    const std::string& filename, sf::Texture& texture)
{
...
void ResourceManager::loadResourceFromFile(
    const std::string& filename, sf::Font& font)
{
...
void ResourceManager::loadResource(const std::string& filename)
{
    ...
    if (!resource)
    {
        throw dgm::EnvironmentException(
            "Could not allocate memory for resource");
    }
    ...
    catch (std::exception& e)
    {
        ...
        throw dgm::GeneralException(std::format(
            "Could not load resource '{}', reason: '{}'", filename, e.what()));
    }
    ...
    {
        delete resource;
        throw dgm::ResourceException(
            std::format("Resource called '{}' is already in database!", id));
    }
    ...
    catch (...)
    {
        delete resource;
        throw dgm::EnvironmentException(
            "Could not insert resource into database");
    }
}

template void
ResourceManager::loadResource<sf::Texture>(const std::string& filename);
template void
ResourceManager::loadResource<sf::SoundBuffer>(const std::string& filename);
template void
    ...
    bool recursive)
{
    ...
    {
        throw dgm::ResourceException(
            std::format("Path '{}' does not exist!", folderPath));
    }
    ...
    for (auto item : itr)
    {
        ...
        if (fs::is_directory(itemPath))
        {
            if (recursive)
                loadResourceDir<T>(
                    itemPath.generic_string(), allowedExtensions, recursive);
            else
                continue;
        }
