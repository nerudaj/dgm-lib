...
dgm::Clip dgm::JsonLoader::loadClipFromStream(std::istream& stream) const
{
	...
	const sf::Vector2u spacing = json.contains("spacing")
		...
		: sf::Vector2u(0u, 0u);
	const unsigned nframes = json.contains("nframes")
		? json["nframes"].get<unsigned>()
		: 0;
...
std::shared_ptr<dgm::AnimationStates> dgm::JsonLoader::loadAnimationsFromStream(std::istream& stream) const
{
	...
	for (auto&& state : file["states"])
	{
		...
		const sf::Vector2u spacing = state.contains("spacing")
			...
			: defaultSpacing;
		const unsigned frameCount = state.contains("nframes")
			? state["nframes"].get<unsigned>()
			: 0;
		const sf::IntRect bounds = getBoundsFromJson(state["bounds"]);

		if (frame.x == 0u || frame.y == 0u)
		{
			throw dgm::ResourceException(std::format("No frame was defined either in defaults block or for state {}", name));
		}
