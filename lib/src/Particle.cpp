...
void dgm::ps::Particle::setRotation(const float angle) noexcept {
	...
	// and create a unit vector that has been rotated so it points to position of first vertex after rotation
	// and is scaled to point to one of the new vertex positions
	const auto POS = getPosition();
	const auto BASE_VEC = dgm::Math::rotateVector(VEC_45_DEG, rotation) * diagonalHalfLength;
	
	// So the first vertex is just offset from center POS in direction of BASE_VEC
	// Each subsequent vertex is perpendicular to the previous one so we do the same and just alter
	// signs and order of X and Y components
