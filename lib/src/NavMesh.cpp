...
dgm::Path<dgm::WorldNavpoint> dgm::WorldNavMesh::getPath(const sf::Vector2f& from, const sf::Vector2f& to)
{
	...
	// Make auxiliary connections to rest of network, unless source/target point is already a jump point
	...
	{
		// Erase auxiliary connections created earlier, but only if from/to points weren't
		// jump points to begin with
		if (not toIsJumpPoint)
		{
